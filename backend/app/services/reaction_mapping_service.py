...
class ReactionMappingService:
    ...
    def map_reaction(
        ...
        for ordered_reactants in permutations(reactants):
            ...
            for reactant_idx, reactant_mol in enumerate(candidate_mols):
                for atom in reactant_mol.GetAtoms():
                    atom.SetIntProp("reactant_idx", reactant_idx)
            raw_product_sets = reaction.RunReactants(tuple(candidate_mols))
            try:
                selected_products = self._find_matching_product_set(raw_product_sets, products)
            except ValueError:
                continue
            reactant_mols = candidate_mols
            break
