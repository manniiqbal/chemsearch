    ...
    return (
        <section className="form-section">
            <div className="form-heading">
                <div><h3>{title}</h3>{helper && <p>{helper}</p>}</div>
                <button className="text-button" type="button" onClick={() => onChange([...values, ""])}>+ Add chemical</button>
            </div>
...
export function ConditionsForm({ value, onChange }: { value: ReactionConditions; onChange: (value: ReactionConditions) => void }) {
    ...
    return (
        <section className="form-section compact-section">
            <div className="form-heading"><div><h3>Conditions</h3><p>Optional details such as temperature or solvent</p></div></div>
